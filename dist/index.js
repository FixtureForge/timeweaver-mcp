...
server.tool("list_presets", "List the built-in time-series presets (realistic ready-made configurations like e-commerce sales, server CPU, IoT temperature, website traffic, stock price, API latency). Use a preset name with generate_timeseries to get sensible defaults you can still override.", {}, async () => {
    const lines = Object.entries(PRESETS).map(([name, p]) => `• ${name} — ${p.description} [frequency: ${p.frequency}]`);
    return {
        content: [
            {
                type: "text",
...
server.tool("generate_timeseries", "Generate realistic synthetic time-series data with configurable trend, seasonality, noise, anomalies, and multiple correlated series. Ideal for testing dashboards, charts, monitoring/alerting, forecasting and anomaly-detection. Output as JSON, CSV, or SQL INSERTs. Use a preset for quick sensible defaults, or specify components explicitly.", {
    ...
    try {
        ...
        if (args.preset) {
            ...
            if (!p) {
                return gateError(`Unknown preset '${args.preset}'. Call list_presets to see available names.`);
            }
        ...
        const spec = { ...presetSeries };
        if (args.baseline !== undefined)
            spec.baseline = args.baseline;
        if (args.trend !== undefined)
            spec.trend = args.trend;
        if (args.trend_strength !== undefined)
            spec.trendStrength = args.trend_strength;
        if (args.seasonality !== undefined)
            spec.seasonality = args.seasonality;
        if (args.noise !== undefined)
            spec.noise = args.noise;
        if (args.noise_level !== undefined)
            spec.noiseLevel = args.noise_level;
        if (args.ar1_phi !== undefined)
            spec.ar1Phi = args.ar1_phi;
        if (args.anomalies !== undefined)
            spec.anomalies = args.anomalies;
        if (args.integer !== undefined)
            spec.integer = args.integer;
        if (args.min !== undefined)
            spec.min = args.min;
        if (args.max !== undefined)
            spec.max = args.max;
        const length = args.length ?? 100;
        const frequency = args.frequency ?? presetFrequency ?? "daily";
        const seriesCount = args.series_count ?? 1;
        const fmt = args.format ?? "json";
        // ---- Enforce free-tier limits -------------------------------------
        if (!pro) {
            if (length > FREE_MAX_LENGTH) {
                return gateError(`Free tier supports up to ${FREE_MAX_LENGTH} points; you requested ${length}. Upgrade to Pro for long series (up to 100k points).`);
            }
            if (seriesCount > FREE_MAX_SERIES) {
                return gateError(`Free tier generates a single series; you requested ${seriesCount}. Upgrade to Pro for multiple correlated series.`);
            }
        ...
        for (let i = 0; i < (pro ? seriesCount : 1); i++) {
            ...
            if (args.names && args.names[i])
                copy.name = args.names[i];
            series.push(copy);
        }
        const options = {
            length,
        ...
        if (fmt === "csv")
            ...
            output = toSQL(result, args.table_name ?? "timeseries");
        else
