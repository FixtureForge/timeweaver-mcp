...
// ---- Timestamps -----------------------------------------------------------
const STEP_MS: Record<Exclude<Frequency, "monthly">, number> = {
  secondly: 1000,
  minutely: 60_000,
  hourly: 3_600_000,
  daily: 86_400_000,
...
function trendComponent(
  ...
  switch (s.trend) {
    ...
    case "logistic": {
      ...
      const mid = length / 2;
      const f = (x: number) => capacity / (1 + Math.exp(-steep * (x - mid)));
...
function seasonalComponent(s: SeriesSpec, t: number): number {
  if (!s.seasonality || s.seasonality.length === 0) return 0;
  let sum = 0;
  for (const comp of s.seasonality) {
    if (comp.period <= 0) continue;
...
function applyAnomalies(
  anomalies: AnomalySpec[] | undefined,
  vals: number[],
  rng: () => number,
  noiseLevel: number,
  baseline: number,
  ...
  for (const a of anomalies) {
    ...
    switch (a.type) {
      ...
      case "dropout": {
        for (let i = 0; i < count; i++) {
          const idx = a.at ?? Math.floor(rng() * length);
          if (idx >= 0 && idx < length) vals[idx] = floor;
        }
        break;
      }
      case "level_shift": {
        const idx = a.at ?? Math.floor(length / 2);
        for (let t = idx; t < length; t++) vals[t] += amount;
        break;
      }
      case "trend_change": {
        const idx = a.at ?? Math.floor(length / 2);
        const perStep = (a.magnitude ?? 0.1) * scale;
        for (let t = idx; t < length; t++) vals[t] += perStep * (t - idx);
        break;
      }
    }
  }
...
export function generate(opts: GenerateOptions): GenerateResult {
  ...
  const rng = mulberry32(
    opts.seed !== undefined ? opts.seed : Math.floor(Math.random() * 2 ** 31)
  );
  const c = Math.max(0, Math.min(1, opts.correlation ?? 0));

  // Shared latent factor sequence used to correlate noise across series.
  const common: number[] = new Array(length);
  for (let t = 0; t < length; t++) common[t] = gaussian(rng);
  ...
  series.forEach((s, i) => {
    ...
    let suffix = 2;
    while (seen.has(nm)) nm = `${s.name ?? `series_${i + 1}`}_${suffix++}`;
    seen.add(nm);
    seriesNames.push(nm);
  });

  const valuesPerSeries: number[][] = [];
  for (let i = 0; i < series.length; i++) {
    ...
    const eps: number[] = new Array(length);
    ...
    const noiseSeq: number[] = new Array(length);
    if (useAR) {
      noiseSeq[0] = eps[0];
      ...
      for (let t = 0; t < length; t++) noiseSeq[t] = eps[t];
    }
    ...
    for (let t = 0; t < length; t++) {
      if (s.min !== undefined) vals[t] = Math.max(s.min, vals[t]);
      if (s.max !== undefined) vals[t] = Math.min(s.max, vals[t]);
...
// ---- Output formatters ----------------------------------------------------
export function toJSON(r: GenerateResult): string {
  const rows = r.points.map((p) => ({ timestamp: p.timestamp, ...p.values }));
  return JSON.stringify(rows, null, 2);
}

export function toCSV(r: GenerateResult): string {
  const header = ["timestamp", ...r.seriesNames].join(",");
  const lines = r.points.map((p) =>
    [p.timestamp, ...r.seriesNames.map((n) => p.values[n])].join(",")
  );
...
export function toSQL(r: GenerateResult, table = "timeseries"): string {
...
export const PRESETS: Record<string, Preset> = {
  ecommerce_sales: {
    description:
      "Daily e-commerce sales: upward trend, weekly + yearly seasonality, occasional promo spikes.",
    frequency: "daily",
    series: {
      baseline: 1000,
      trend: "linear",
  ...
  server_cpu: {
    description:
      "Per-minute server CPU %: autocorrelated noise around a baseline with periodic load and occasional spikes (clamped 0-100).",
    frequency: "minutely",
    series: {
      baseline: 35,
  ...
  iot_temperature: {
    description:
      "Hourly IoT temperature sensor: daily cycle with slow upward drift and small sensor noise.",
    frequency: "hourly",
    series: {
      baseline: 21,
      trend: "linear",
  ...
  website_traffic: {
    description:
      "Hourly website visits: daily + weekly seasonality with a growth trend.",
    frequency: "hourly",
    series: {
      baseline: 500,
      trend: "linear",
  ...
  stock_price: {
    description:
      "Daily stock-like price: persistent random-walk drift with volatility, no seasonality.",
    frequency: "daily",
    series: {
      baseline: 100,
      noise: "ar1",
  ...
  api_latency_ms: {
    description:
      "Per-minute API latency (ms): low baseline with frequent latency spikes.",
    frequency: "minutely",
    series: {
      baseline: 80,
      noiseLevel: 15,
